...
namespace AspNet.Identity.RavenDB.Stores
{
        ...
        IDisposable where TUser : RavenUser
    {
        ...
        public Task SetPhoneNumberAsync(TUser user, string phoneNumber, string type)
        {
            if (user == null) throw new ArgumentNullException("user");
            if (phoneNumber == null) throw new ArgumentNullException("phoneNumber");

            user.SetPhoneNumber(phoneNumber, type);
            RavenUserPhoneNumber ravenUserPhoneNumber = new RavenUserPhoneNumber(phoneNumber, user.Id);
        ...
        {
            return _documentSession.Query<RavenUserRole>().Where(r => r.userId == user.Id).Select(r => r.roleId).ToListAsync();
        }

        public Task<bool> IsInRoleAsync(TUser user, string roleName)
        {

            return Task.FromResult(_documentSession.Query<RavenUserRole>().Count(r => r.userId == user.Id && r.roleId == roleName) > 0);

        }

        public async Task RemoveFromRoleAsync(TUser user, string roleName)
        {
            RavenUserRole userRole = _documentSession.Query<RavenUserRole>().Where(r => r.roleId == roleName && r.userId == user.Id).FirstOrDefault();
            _documentSession.Delete<RavenUserRole>(userRole);
            await _documentSession.SaveChangesAsync();
